...
// MEM_REGIONS is a list, and each element within it is of mem_region_t type. So here....Maybe this MEM_NREGIONS=5? (Yes)
...
void load_program(char *program_filename) {                   
  ...
  /* Read in the program. */
  printf("In load_program\n");
  ii = 0;
  while (fscanf(prog, "%x\n", &word) != EOF) {  // read x (hex number) from prog (file pointer) and write into word. Interestingly, it will read the value line by line. 
    printf("word is %d\n", word);
