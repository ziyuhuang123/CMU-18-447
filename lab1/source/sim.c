...
void process_instruction() {
     ...
     * access memory. */
     ...
     strncpy(instruction, conversion, 33);  // copy the content of 'conversion' to 'instruction'. 
     printf("%s\n", instruction);
     if(convert(32) == 12) {  // And 'instruction' is a global variable, so convert function can visit it. 
        if(NEXT_STATE.REGS[2] == 10) {  // This is 'SYSCALL', as especially mentioned in the pdf file. 
            RUN_BIT = 0;
            return;
        }
    }
    position = 0;
    int type = convert(6);  // Get the first six number, which is the op code. 
    if(type == 0) { // R - TYPE
        int rs = convert(5), rt = convert(5), rd = convert(5), shamt = convert(5), funct = convert(6);  // Here, convert(5) means take this five binary number, and then, rt will take next five number and then rd. 
        switch (funct) {
            case 32: // ADD
